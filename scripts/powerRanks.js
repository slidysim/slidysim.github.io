// Function to determine the tier of a score based on its time and index
function getScoreTier(time, index, tiers) {
    // Iterate tiers in reverse order (highest to lowest)
    for (let i = tiers.length - 1; i >= 0; i--) {
        const tier = tiers[i];
        if (time <= tier.times[index]) { // Compare with the time at the specific index
            return tier;
        }
    }
    return tiers[0]; // Default to the lowest tier (Unranked)
}

// Function to calculate player power and extract times
function calculatePlayerPower(savedPlayerScores, tiers) {
    ...
    for (const player of savedPlayerScores) {
        ...
        // Extract times and calculate power
        player.scores.forEach((score, index) => {
            const time = score.scoreInfo.time;
            if (typeof time !== 'number' || isNaN(time)) {
                playerTimes.push(-1); // Push -1 as a fallback value
            } else {
                playerTimes.push(time); // Push the valid time value
            }

            // Determine the tier for this score using the score's index
    ...
    // Sort players by total power (descending)
    players.sort((a, b) => b.totalPower - a.totalPower);
...
function getPowerData(){
    ...
    sendMyRequest();
    let powerData = calculatePlayerPower(savedPlayerScores, tiers);
    ...
    iframe.onload = () => {
        iframe.contentWindow.postMessage(powerData, '*');
    }
    loadingPower = false;
}